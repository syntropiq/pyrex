...
describe('Python Backend - Regex (converted)', () => {
  ...
  it('regex.search test 2', async () => {
    ...
    expect((await re.search("(znacnda){s<=1, e<=3}", "znacnda"))?.span()).toStrictEqual([9, 17]);
  });
  ...
  it('regex.search test 13', async () => {
    ...
    expect((await re.search("(foobar){e<=2}", "xirefoabrzlfd"))?.span()).toStrictEqual([4, 9]);
  });
  ...
  it('regex.search test 15', async () => {
    ...
    expect((await re.search("(foobar){i<=2, s<=2}", "fooba"))?.span()).toStrictEqual([5, 11]);
  });

  it('regex.search test 16', async () => {
    // Python: # Find best whole word match for "fooba".self.assertEqual(regex.search("\\x08(foobar){e}\\", "zfoobarz").span(0, 1), ((0, 8), (0, 8))
    expect(await re.search("\\\\x08(foobar){e}\\\\", "zfoobarz")?.span()).toStrictEqual(((0, 8);
  });

  it('regex.search test 17', async () => {
    // Python: self.assertEqual(regex.search("\\x08(foobar){e}\\", "boing zfoobarz goobar woop").span(0, 1),((0, 6), (0, 6)),)
    expect(await re.search("\\\\x08(foobar){e}\\\\", "boing zfoobarz goobar woop")?.span()).toStrictEqual(((0, 6);
  });

  it('regex.search test 18', async () => {
    // Python: self.assertEqual(regex.search("(?b)\\x08(foobar){e}\\", "boing zfoobarz goobar woop").span(0, 1),((15, 21), (15, 21)),)
    expect(await re.search("(?b)\\\\x08(foobar){e}\\\\", "boing zfoobarz goobar woop")?.span()).toStrictEqual(((15, 21);
  });

  it('regex.search test 19', async () => {
    // Python: # Match whole string, allow only 1 error.self.assertEqual(regex.search("^(foobar){e<=1}$", "fooba").span(0, 1), ((0, 6), (0, 6))
    expect((await re.search("^(foobar){e<=1}$", "fooba"))?.span()).toStrictEqual([0, 6]);
  });

  it('regex.search test 20', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "xfooba").span(0, 1), ((0, 7), (0, 7))
    expect((await re.search("^(foobar){e<=1}$", "xfooba"))?.span()).toStrictEqual([0, 7]);
  });

  it('regex.search test 21', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "foobarx").span(0, 1), ((0, 7), (0, 7))
    expect((await re.search("^(foobar){e<=1}$", "foobarx"))?.span()).toStrictEqual([0, 7]);
  });

  it('regex.search test 22', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "fooxba").span(0, 1), ((0, 7), (0, 7))
    expect((await re.search("^(foobar){e<=1}$", "fooxba"))?.span()).toStrictEqual([0, 7]);
  });

  it('regex.search test 23', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "foxba").span(0, 1), ((0, 6), (0, 6))
    expect((await re.search("^(foobar){e<=1}$", "foxba"))?.span()).toStrictEqual([0, 6]);
  });

  it('regex.search test 24', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "xooba").span(0, 1), ((0, 6), (0, 6))
    expect((await re.search("^(foobar){e<=1}$", "xooba"))?.span()).toStrictEqual([0, 6]);
  });

  it('regex.search test 25', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "foobax").span(0, 1), ((0, 6), (0, 6))
    expect((await re.search("^(foobar){e<=1}$", "foobax"))?.span()).toStrictEqual([0, 6]);
  });

  it('regex.search test 26', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "ooba").span(0, 1), ((0, 5), (0, 5))
    expect((await re.search("^(foobar){e<=1}$", "ooba"))?.span()).toStrictEqual([0, 5]);
  });

  it('regex.search test 27', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "foba").span(0, 1), ((0, 5), (0, 5))
    expect((await re.search("^(foobar){e<=1}$", "foba"))?.span()).toStrictEqual([0, 5]);
  });

  it('regex.search test 28', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "fooba").span(0, 1), ((0, 5), (0, 5))
    expect((await re.search("^(foobar){e<=1}$", "fooba"))?.span()).toStrictEqual([0, 5]);
  });
  ...
  it('regex.search test 31', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "xxfooba"), null)
    expect(await re.search("^(foobar){e<=1}$", "xxfooba")).toStrictEqual(null);
  });

  it('regex.search test 32', async () => {
    // Python: self.assertEqual(regex.search("^(foobar){e<=1}$", "xfoxba"), null)
    expect(await re.search("^(foobar){e<=1}$", "xfoxba")).toStrictEqual(null);
  });
  ...
  it('regex.search test 34', async () => {
    ...
    expect((await re.search("(foobar){i<=1, d<=2}", "fooba"))?.span()).toStrictEqual([6, 13]);
  });
  ...
  it('regex.search test 35', async () => {
    ...
    expect((await re.search("(?b)(foobar){i<=1, d<=2}", "fooba"))?.span()).toStrictEqual([34, 39]);
  });
  ...
  it('regex.search test 36', async () => {
    ...
    expect((await re.search("foo(bar){e<=1}zap", "foobarzap"))?.span()).toStrictEqual([0, 9]);
  });
  ...
  it('regex.search test 38', async () => {
    ...
    expect((await re.search("foo(bar){e<=1}zap", "foobrzap"))?.span()).toStrictEqual([0, 8]);
  });

  it('regex.search test 39', async () => {
    // Python: self.assertEqual(regex.search("(?s)^.*(dot.org){e}.*$", text).span(0, 1),((0, 120), (120, 120)),)
    const text = `
        This is a long string with dot.org and other text.
        It spans multiple lines to test dotall flag.
        Another line with dot.org.
        Final line.
    `;
    expect((await re.search("(?s)^.*(dot.org){e}.*$", text))?.span()).toStrictEqual([0, 120]);
  });

  it('regex.search test 40', async () => {
    // Python: self.assertEqual(regex.search("(?es)^.*(dot.org){e}.*$", text).span(0, 1),((0, 120), (93, 100)),)
    expect((await re.search("(?es)^.*(dot.org){e}.*$", text))?.span()).toStrictEqual([0, 120]);
  });

  it('regex.search test 41', async () => {
    // Python: self.assertEqual(regex.search("^.*(dot.org){e}.*$", text).span(0, 1), ((0, 119), (24, 101))
    expect((await re.search("^.*(dot.org){e}.*$", text))?.span()).toStrictEqual([0, 119]);
  });

  it('regex.findall test 42', async () => {
    // Python: self.assertEqual(regex.findall("(?e)\x08\L<words>{e<=1}\"," book dog cot desk ",words="cat dog".split(),),["dog", "cot"],)
    expect(await re.findall("(?e)\\x08\\L<words>{e<=1}\\", " book dog cot desk ")).toStrictEqual(["dog", "cot"]);
  });

  it('regex.findall test 43', async () => {
    // Python: self.assertEqual(regex.findall("(?r)\x08\L<words>{e<=1}\"," book cot dog desk ",words="cat dog".split(),),["dog ", "cot"],)
    expect(await re.findall("(?r)\\x08\\L<words>{e<=1}\\", " book cot dog desk ")).toStrictEqual(["dog ", "cot"]);
  });

  it('regex.findall test 44', async () => {
    // Python: self.assertEqual(regex.findall("(?er)\x08\L<words>{e<=1}\"," book cot dog desk ",words="cat dog".split(),),["dog", "cot"],)
    expect(await re.findall("(?er)\\x08\\L<words>{e<=1}\\", " book cot dog desk ")).toStrictEqual(["dog", "cot"]);
  });

  it('regex.findall test 45', async () => {
    // Python: self.assertEqual(regex.findall("(?r)\x08\L<words>{e<=1}\"," book dog cot desk ",words="cat dog".split(),),["cot", "dog"],)
    expect(await re.findall("(?r)\\x08\\L<words>{e<=1}\\", " book dog cot desk ")).toStrictEqual(["cot", "dog"]);
  });

  it('regex.findall test 46', async () => {
    // Python: self.assertEqual(regex.findall("\x08\L<words>{e<=1}\"," book cot dog desk ",words="cat dog".split(),),["cot", "dog"],)
    expect(await re.findall("\\x08\\L<words>{e<=1}\\", " book cot dog desk ")).toStrictEqual(["cot", "dog"]);
  });

  it('regex.findall test 47', async () => {
    // Python: self.assertEqual(regex.findall("\x08\L<words>{e<=1}\"," book dog cot desk ",words="cat dog".split(),),[" dog", "cot"],)
    expect(await re.findall("\\x08\\L<words>{e<=1}\\", " book dog cot desk ")).toStrictEqual([" dog", "cot"]);
  });
  ...
  it('regex.findall test 51', async () => {
    // Python: self.assertEqual(regex.findall("(?r)\x08\L<words>{e<=1}\"," book dog cot desk ",words="cat dog".split(),),["cot", "dog"],)
    expect(await re.findall("\\x08\\L<words>{e<=1}\\", " book dog cot desk ")).toStrictEqual(["cot", "dog"]);
  });

  it('regex.search test 52', async () => {
    // Python: self.assertEqual(regex.search("(\w+) (\1{e<=1})", "foo fou").groups(), ("foo", "fou")
    expect((await re.search("(\\w+) (\\1{e<=1})", "foo fou"))?.groups()).toStrictEqual(["foo", "fou"]);
  });

  it('regex.search test 53', async () => {
    // Python: self.assertEqual(regex.search("(?r)(\2{e<=1}) (\w+)", "foo fou").groups(), ("foo", "fou")
