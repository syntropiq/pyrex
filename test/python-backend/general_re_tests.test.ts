...
describe('Python Backend - Regex (converted)', () => {
  ...
  it('regex.findall test 1', async () => {
    ...
    expect(await re.findall(":+", "abc", { backend: 'python' })).toStrictEqual([]);
  });

  it('regex.findall test 2', async () => {
    // Python: self.assertEqual(regex.findall(":+", "a:b::c:::d"), [':', '::', ':::'])
    expect(await re.findall(":+", "a:b::c:::d", { backend: 'python' })).toStrictEqual([':', '::', ':::']);
  });
  ...
  it('regex.findall test 18', async () => {
    ...
    expect(await re.findall("(?i)(.{1, 40}?, { backend: 'python' })).toStrictEqual((.{1,40}?);
  });
  ...
  it('regex.match test 19', async () => {
    ...
    expect(await re.match("^(\\w){1}$", "abc", { backend: 'python' })).toStrictEqual(None);
  });
  ...
  it('regex.match test 21', async () => {
    ...
    expect(await re.match("^(\\w){1, 2}$", { backend: 'python' })).toStrictEqual(None);
  });
  ...
  it('regex.match test 23', async () => {
    ...
    expect(await re.match("^x{1}$", "xxx", { backend: 'python' })).toStrictEqual(None);
  });
  ...
  it('regex.match test 25', async () => {
    ...
    expect(await re.match("^x{1, 2}$", { backend: 'python' })).toStrictEqual(None);
  });
  ...
  it('regex.match test 27', async () => {
    ...
    expect(await re.match("^x{}$", "xxx", { backend: 'python' })).toStrictEqual(None);
  });
...
// Skipped test 35: incomplete arguments in Python: self.assertEqual(regex.sub('(.)', regex.escape(r"\1\1"), 'x'), r"\1\1")
  ...
  it('regex.sub test 36', async () => {
    ...
    expect(await re.sub("(.)", "\\\\1\\\\1", "x", { backend: 'python' })).toBe(r"\1\1");
  });

  it('regex.sub test 37', async () => {
    // Python: self.assertEqual(regex.sub('(.)', lambda m: r"\1\1", 'x'), r"\1\1")
    expect(await re.sub("(.)", // TODO: Manual conversion needed for callable replacement, "x", { backend: 'python' })).toBe(r"\1\1");
  });

  it('regex.search test 38', async () => {
    // Python: self.assertEqual(regex.search(r"(\w)(?:(?R)|(\w?))\1", "dontmatchme"),None)
    expect(await re.search("(\\w)(?:(?R)|(\\w?))\\1", "dontmatchme", { backend: 'python' })).toStrictEqual(None);
  });

  it('regex.search test 39', async () => {
    // Python: self.assertEqual(regex.search(r"(?r)\2(?:(\w?)|(?R))(\w)","dontmatchme"), None)
    expect(await re.search("(?r)\\2(?:(\\w?)|(?R))(\\w)", "dontmatchme", { backend: 'python' })).toStrictEqual(None);
  });

  it('regex.search test 40', async () => {
    // Python: self.assertEqual(regex.search(r"\(((?>[^()]+)|(?R))*\)","(ab(cd)ef)").captures(1), ["ab", "cd", "(cd)", "ef"])
    expect(await re.search("\\(((?>[^()]+)|(?R))*\\)", "(ab(cd)ef)", { backend: 'python' })?.captures(1)).toStrictEqual(["ab", "cd", "(cd);
  });

  it('regex.search test 41', async () => {
    // Python: self.assertEqual(regex.search(r"(?r)\(((?R)|(?>[^()]+))*\)","(ab(cd)ef)").captures(1), ["ef", "cd", "(cd)", "ab"])
    expect(await re.search("(?r)\\(((?R)|(?>[^()]+))*\\)", "(ab(cd)ef)", { backend: 'python' })?.captures(1)).toStrictEqual(["ef", "cd", "(cd);
  });
});
