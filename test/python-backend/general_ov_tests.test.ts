...
describe('Python Backend - Regex (converted)', () => {
  ...
  it('regex.findall test 1', async () => {
    // Python: import regeximport unittestclass TestGeneralOv(unittest.TestCase):def test_overlapped(self):self.assertEqual(regex.findall(r"..", "abcde"), ["a", "cd"])
    expect(await re.findall("..", "abcde")).toStrictEqual(["a", "cd"]);
  });

  it('regex.findall test 2', async () => {
    // Python: self.assertEqual(regex.findall(r"..", "abcde", overlapped=True), ["a", "bc", "cd", "de"])
  ...
  it('regex.findall test 5', async () => {
    // Python: self.assertEqual(regex.findall(r"(.)(-)(.)", "a-b-c", overlapped=True),[("a", "-", ""), ("", "-", "c")],)
