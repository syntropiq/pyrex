...
describe('Python Backend - Regex (converted)', () => {
  ...
  it('regex.match test 1', async () => {
    ...
    expect(await re.match("(?P<first>first) (?P<second>second)", "first second")?.groupdict()).toStrictEqual({"first": "first", "second": "second"},);
  });

  it('regex.match test 2', async () => {
    // Python: # test_graphemedef test_grapheme(self):self.assertEqual(regex.match("\X", "\xe0").span(), (0, 1))
