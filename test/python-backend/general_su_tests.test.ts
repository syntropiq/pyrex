...
describe('Python Backend - Regex (converted)', () => {
  ...
  it('regex.sub test 19', async () => {
    ...
    expect(await re.sub("(((((((((((x)))))))))))", "\\11", "x", { backend: 'python' })).toBe('x');
  });

  it('regex.sub test 20', async () => {
    // Python: self.assertEqual(regex.sub('((((((((((y))))))))))(.)', r'\118', 'xyz'),'xz8')
    expect(await re.sub("((((((((((y))))))))))(.)", "\\118", "xyz", { backend: 'python' })).toBe('xz8');
  });

  it('regex.sub test 21', async () => {
    // Python: self.assertEqual(regex.sub('((((((((((y))))))))))(.)', r'\11a', 'xyz'),'xza')
    expect(await re.sub("((((((((((y))))))))))(.)", "\\11a", "xyz", { backend: 'python' })).toBe('xza');
  });
  ...
  it('regex.match test 22', async () => {
    ...
    expect(await re.match("(?P<x>.)+", "abc", { backend: 'python' })?.expandf('{0} {0[0]} {0[-1]}')).toStrictEqual('abc abc abc');
  });
  ...
  it('regex.match test 23', async () => {
    ...
    expect(await re.match("(?P<x>.)+", "abc", { backend: 'python' })?.expandf('{1} {1[0]} {1[1]} {1[2]} {1[-1]} {1[-2]} {1[-3]}')).toStrictEqual('c a b c c b a');
  });
  ...
  it('regex.match test 24', async () => {
    ...
    expect(await re.match("(?P<x>.)+", "abc", { backend: 'python' })?.expandf('{x} {x[0]} {x[1]} {x[2]} {x[-1]} {x[-2]} {x[-3]}')).toStrictEqual('c a b c c b a');
  });
});
