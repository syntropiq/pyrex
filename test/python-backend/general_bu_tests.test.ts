...
describe('Python Backend - Regex (converted)', () => {
  ...
  it('regex.sub test 1', async () => {
    ...
    expect(await re.sub("(?P<unk>x)", "\\g<1>\\g<1>\\b", "xx", { backend: 'python' })).toBe("xx\bxx\b");
  });
  ...
  it('regex.sub test 6', async () => {
    ...
    expect(await re.sub("(\\S)\\s+(\\S)", "\\1 \\2", "hello  there", { backend: 'python' })).toBe("hello there");
  });
  ...
  it('regex.sub test 7', async () => {
    ...
    expect(await re.sub("(?V0)x*", "-", "abxd", { backend: 'python' })).toBe("-a-b--d-");
  });
  ...
  it('regex.sub test 9', async () => {
    ...
    expect(await re.sub("(?V1)x*", "-", "abxd", { backend: 'python' })).toBe("-a-b--d-");
  });

  it('regex.sub test 10', async () => {
    // Python: self.assertEqual(regex.sub("x+", "-", "abxd"), "ab-d")
    expect(await re.sub("x+", "-", "abxd", { backend: 'python' })).toBe("ab-d");
  });
  ...
  it('regex.search test 11', async () => {
    ...
    expect(await re.search("(?P<" + group_name + ">a)", "abc", { backend: 'python' })?.group(group_name)).toStrictEqual("a");
  });
  ...
  it('regex.findall test 12', async () => {
    ...
    expect(await re.findall("(a|(b))", "aba", { backend: 'python' })).toStrictEqual([("a", "");
  });
  ...
  it('regex.match test 13', async () => {
    ...
    expect(await re.match("(a)|(b)", "b", { backend: 'python' })?.start(1)).toStrictEqual(-1);
  });

  it('regex.match test 14', async () => {
    // Python: self.assertEqual(regex.match(r"(a)|(b)", "b").end(1), -1)
    expect(await re.match("(a)|(b)", "b", { backend: 'python' })?.end(1)).toStrictEqual(-1);
  });

  it('regex.match test 15', async () => {
    // Python: self.assertEqual(regex.match(r"(a)|(b)", "b").span(1), (-1, -1))
    expect(await re.match("(a)|(b)", "b", { backend: 'python' })?.span(1)).toStrictEqual((-1, -1);
  });
  ...
  it('regex.match test 16', async () => {
    ...
    expect(await re.match(".*?c", 10000 * "ab" + "cd", { backend: 'python' })?.end(0)).toStrictEqual(20001);
  });

  it('regex.match test 17', async () => {
    // Python: self.assertEqual(regex.match(".*?cd", 5000 * "ab" + "c" + 5000 * "ab" + "cde").end(0), 20003)
    expect(await re.match(".*?cd", 5000 * "ab" + "c" + 5000 * "ab" + "cde", { backend: 'python' })?.end(0)).toStrictEqual(20003);
  });

  it('regex.match test 18', async () => {
    // Python: self.assertEqual(regex.match(".*?cd", 20000 * "abc" + "de").end(0), 60001)
    expect(await re.match(".*?cd", 20000 * "abc" + "de", { backend: 'python' })?.end(0)).toStrictEqual(60001);
  });
  ...
  it('regex.search test 19', async () => {
    ...
    expect(await re.search("(a|b)*?c", 10000 * "ab" + "cd", { backend: 'python' })?.end(0)).toStrictEqual(20001);
  });
