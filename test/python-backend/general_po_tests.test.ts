...
describe('Python Backend - Regex (converted)', () => {
  ...
  it('regex.search test 5', async () => {
    ...
    expect(await re.search("(?:ab)?ab", "ab")?.span()).toStrictEqual((0, 2);
  });

  it('regex.search test 6', async () => {
    // Python: self.assertEqual(regex.search(r"(?:ab)*ab", "ababab").span(), (0, 6))
    expect(await re.search("(?:ab)*ab", "ababab")?.span()).toStrictEqual((0, 6);
  });
  ...
  it('regex.search test 16', async () => {
    ...
    expect(await re.search("(?:ab){1, 3}+ab")).toStrictEqual(None);
  });
});
