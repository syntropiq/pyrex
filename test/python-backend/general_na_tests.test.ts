...
describe('Python Backend - Regex (converted)', () => {
  ...
  it('regex.match test 1', async () => {
    ...
    expect(await re.match("333\\L<bar>444", "333one444", { backend: 'python' })?.group()).toStrictEqual("333one444");
  });

  it('regex.match test 2', async () => {
    // Python: self.assertEqual(regex.match(r"(?i)333\L<bar>444", "333TWO444", bar=options).group(),"333TWO444",)
    expect(await re.match("(?i)333\\L<bar>444", "333TWO444", { backend: 'python' })?.group()).toStrictEqual("333TWO444",);
  });

  it('regex.match test 3', async () => {
    // Python: self.assertEqual(regex.match(r"333\L<bar>444", "333four444", bar=options), None)
    expect(await re.match("333\\L<bar>444", "333four444", { backend: 'python' })).toStrictEqual(None);
  });
  ...
  it('regex.findall test 7', async () => {
    ...
    expect(await re.findall("^\\L<options>", "solid QWERT", { backend: 'python' })).toStrictEqual([],);
  });
  ...
  it('regex.findall test 8', async () => {
    ...
    expect(await re.findall("^\\L<options>", "+solid QWERT", { backend: 'python' })).toStrictEqual(["+solid"],);
  });
  ...
  it('regex.match test 9', async () => {
    ...
    expect(await re.match("(?fi)\\L<words>", "stra\\N{LATIN SMALL LETTER SHARP S}e", { backend: 'python' })?.span()).toStrictEqual((0, 6);
  });
  ...
  it('regex.match test 11', async () => {
    ...
    expect(await re.match("(?fi)\\L<words>", "STRASSE", { backend: 'python' })?.span()).toStrictEqual((0, 7);
  });

  it('regex.search test 12', async () => {
    // Python: options = ["kit"]self.assertEqual(regex.search(r"(?i)\L<words>", "SKITS", words=options).span(), (1, 4)
    expect(await re.search("(?i)\\L<words>", "SKITS", { backend: 'python' })?.span()).toStrictEqual((1, 4);
  });
  ...
  it('regex.search test 13', async () => {
    ...
    expect(await re.search("(?i)\\L<words>", "SK\\N{LATIN CAPITAL LETTER I WITH DOT ABOVE}TS", { backend: 'python' })?.span()).toStrictEqual((1, 4);
  });
  ...
  it('regex.search test 14', async () => {
    ...
    expect(await re.search("(?fi)\\b(\\w+) +\\1\\b", " stra\\N{LATIN SMALL LETTER SHARP S}e STRASSE ", { backend: 'python' })?.span()).toStrictEqual((1, 15);
  });
  ...
  it('regex.search test 15', async () => {
    ...
    expect(await re.search("(?fi)\\b(\\w+) +\\1\\b", " STRASSE stra\\N{LATIN SMALL LETTER SHARP S}e ", { backend: 'python' })?.span()).toStrictEqual((1, 15);
  });

  it('regex.search test 16', async () => {
    // Python: self.assertEqual(regex.search(r"^\L<options>$", "", options=[]).span(), (0, 0))
    expect(await re.search("^\\L<options>$", "", { backend: 'python' })?.span()).toStrictEqual((0, 0);
  });
});
