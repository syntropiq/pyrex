...
export interface AsyncPattern {
   ...
   */
  finditer(string: string, pos?: number, endpos?: number): AsyncIterableIterator<Match>;
  
  /**
   * Return the string obtained by replacing the leftmost non-overlapping occurrences
   */
  sub(repl: string | ((match: Match) => string), string: string, count?: number): Promise<string>;
  
  /**
   * Same as sub(), but also return the number of substitutions made
   */
  subn(repl: string | ((match: Match) => string), string: string, count?: number): Promise<[string, number]>;
}