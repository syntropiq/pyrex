...
A TypeScript library providing a Python-like `re` interface for regex operations, targeting evergreen browsers. The API closely mirrors Python’s `re` module, with seamless support for Python regex features through Pyodide.
...
- **Python-only backend**: Uses Pyodide and Python’s `regex` package for all regex operations
- **Efficient implementation**: Patterns are compiled once and stored in a Python-side registry, with all operations using a handle for optimal performance
...
const result = await re.match('a(b+)', 'abbb');
if (result) {
...
- When using Python regex features, patterns are compiled once and stored in a Python-side registry (dict).
